...
- [ ] calendar_event: Google Calendar is SOURCE OF TRUTH; local rows are ASYNC WRITE JOBS + push bookkeeping (pushed_at/push_failed_at), NEVER read as availability; entity types habit|task|weekly_item|daily_adhoc|leisure; completed_at.
- [x] DECIDED (2026-07-20): planners read LIVE availability through the user's EXTERNAL Google Calendar MCP (whole day ahead for daily, whole week ahead for weekly) — enforced by server instructions + planner tool descriptions, with an ask-the-user fallback when no calendar tool is connected. Dream context tools return NO availability data (daily context's local scheduledOnDate read removed; calendarNote points at the rule). The heartbeat retries failed pushes daily (the "worker").
